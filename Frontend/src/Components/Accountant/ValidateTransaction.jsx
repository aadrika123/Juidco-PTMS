...
const ValidateTransaction = () => {
    ...
    return (
        <AccountantDashboard>
            <div>
                <TitleBar titleBarVisibility={titleBarVisibility} titleText={"Transaction validated View Details"} />
            </div>
                                ...
                                {conductorDetails?.data?.length > 0 ? (
                                    conductorDetails.data.map((transaction, index) => (
                                        <tr key={transaction.transaction_id}>
                                            ...
                                            <td className="border border-gray-200 px-4 py-2">{transaction?.total_amount}</td>
                                            <td className="border border-gray-200 px-4 py-2">{transaction?.conductor_id}</td>
                                            <td className="border border-gray-200 px-4 py-2">{transaction?.conductor_name}</td>
                                            <td className="border border-gray-200 px-4 py-2">{new Date(transaction.date).toLocaleDateString()}</td>
                                            <td className="border border-gray-200 px-4 py-2">{transaction.time}</td>
                                            {/* <td className="border border-gray-200 px-4 py-2">{transaction.description}</td> */}
                                            <td className="border border-gray-200 px-4 py-2">{transaction?.transaction_type}</td>
                                            <td className="border border-gray-200 px-4 py-2">{transaction?.bus_id}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
